...
| Reclamation chain (compost + clean water + seed -> grass -> crops -> trees) | Medium | Healing is a supply chain. One machine per rung (nothing renews on its own); the payout is the returning overworld - healed land is the second economy (P2.4-R). **Saplings are never obtainable as items (P2.4-R2, shipped)** - the tree planter is the only source of trees, so a tree farm is a running machine rather than hand-replanting. |
