...
| Pick-through hand-sorting (right-click a placed block for components) | Easy | Loot-table-driven extraction; a Block of Garbage crumbles after 2-3 pulls, a Compacted Bale after 3-4. |
...
| Teardown machine (feed item -> components) | Medium | Shipped 2026-07-16 - the Recompile Workbench (named the Teardown Workbench since Recompile 0.10.0), materials-only and GUI-free (hold-to-disassemble; tools rest on the table). Since Recompile 0.20.0 (P3.10) teardown yields the working component, not knowledge; see the next row. |
| Teardown-as-knowledge (recover recipes) | **Medium** (v1) | **Retired in Recompile 0.20.0 (P3.10):** teardown teaches nothing and every Blueprint is bought at a Buy Terminal. Was THE distinct axis. v1 rode vanilla `doLimitedCrafting` + recipe book grants (~1-2 wks on top of the bench); risks are the JEI locked-recipe overlay and FTB Teams sync. The Hard version (auto recipe introspection, gating other mods' machine recipes) stays maybe-never. Hand-authored JSON unlock tables only. |
| Garbage regions (designed as household / scrapyard / e-waste / slag / hazmat) | Medium | Real biomes, distance-banded from spawn; per-biome garbage blocks. Original plan: launch trio household + scrapyard + e-waste, slag + hazmat in P2. **Shipped as four biomes instead:** `household_sprawl`, `demolition_yard`, `radioactive_dump`, `compacted_depths`. |
| Mound regrowth (mounds regenerate to original footprint, blocks fall from the sky) | Medium | Renewable quarries, not doom. Falling-block delivery from world top (deorbiting garbage). Original-bounds memory settled as a `mound_bed` blockstate carrying the cell's original height - no SavedData. **Shipped in Recompile 0.7.0** (config `moundRegrowthEnabled`, default on). |
...
| Tier 2 processing (Burn Barrel + repaired furnace; purity-as-yield) | Easy-Medium | Burn Barrel **shipped 2026-07-17** (pulled ahead): a manual-only (non-automatable) vanilla-furnace reskin, `scrap -> copper nugget` (copper-first inversion). The repaired furnace remains; the metal-tier split has shipped as the Cupola Furnace, the only machine that makes iron. Fuel: Oily Rag + junk, no energy. |
| Tier 3 logistics (Recompile converts, something else moves) | Easy | **Corrected 2026-09-04:** this said "Create in the pack". Create has no 26.1.2 build and is not here. **Pipez** is what the automation policy is written and tested against; **AE2**, **Ender IO**, **LaserIO** and **Modular Routers** are the shipped logistics. Recompile machines never require any of them. |
| Reclamation chain (grass -> crops -> trees) | Medium | One machine per rung (nothing renews on its own); the payout is the returning overworld - healed land is the second economy (P2.4-R). **Rung 1 is a *drip irrigator* that consumes nothing (P2.4-R3)**, fed by an incorporated Water Tank; the rungs are **multiblocks** built from a shared inert component vocabulary, and the framework is **shipped**. **Corrected 2026-09-04:** this said the Pump is teardown-only and that rung 1 sits behind the teardown spine. The Pump is `blueprint_crafting` as well as a Bulky Waste find, and Recompile 0.18.0 moved the crafted one onto **Rubber Scrap**, whose only source is a tire dump. Rung 1 sits behind the blueprint and the rubber; the found Pump is still a single unit that teaches nothing. **Saplings are never obtainable as items (P2.4-R2, shipped)** - the tree planter is the only source of trees, so a tree farm is a running machine rather than hand-replanting. |
| Hazmat gating (radiation + hazmat suit) | **Unfunded** | **Corrected 2026-09-04:** Mekanism was to own the systems and has no 26.1.2 build. Recompile ships the biome, blocks and caches - the radioactive dump and its uraninite are real - but **no mod in the lineup implements radiation or a hazmat suit**, so the gate does not exist. Either Powah's own uraninite handling carries it or the feature needs cutting. Open. |
| E-waste recovery chains (crude smelt early, chemistry late) | Easy early, **unfunded late** | Two-stage purity-as-yield; battery chain is its own mini-tree and shipped in Recompile 0.17.0. **Corrected 2026-09-04:** the late stage was Mekanism chemistry, which has no 26.1.2 build. The early half stands on Recompile's own Separator. |
| Quest line (FTB Quests, written to the voice source CLAUDE.md names; hidden post-twist chapters) | Easy | 66 quests across four chapters so far. Titled in the exile fiction; the twist delivery vehicle. See the_twist.md. |
...
| Field Manual (technical reference, no custom book) | Easy | **Shipped** as Recompile's Modonomicon guide book (craftable). Use whatever guide-book mod is updated for 26.x; NOT a lore vehicle (minimize-prose principle). |
...
- **Teardown-as-knowledge was the distinct axis and the riskiest build**, though the walkthrough downgraded it P1-Hard -> Medium (it rode vanilla `doLimitedCrafting`). **Recompile 0.20.0 retired it (P3.10):** teardown yields the working component, every Blueprint is bought at a Buy Terminal, and knowledge comes only from the market.
- **Everything in P0 is Easy/Medium.** The thesis can be tested in a week without touching anything hard.
- **The pack leans hard on curation.** The principle holds: the Recompile mod deliberately doesn't build what the lineup already ships, and custom code stays concentrated in worldgen, mound regrowth, and the knowledge system. **Corrected 2026-09-04:** this used to name Create for logistics and Mekanism for chemistry/radiation/energy. Neither has a NeoForge build past 1.21.1, so the curation principle was pointing at two mods that cannot be here. What actually carries those roles: **AE2, Ender IO, Pipez, LaserIO and Modular Routers** for logistics, **Powah** for energy, and **nothing** for chemistry or radiation - which is a real hole, not a substitution.
