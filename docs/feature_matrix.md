...
| Reclamation chain (grass -> crops -> trees) | Medium | One machine per rung (nothing renews on its own); the payout is the returning overworld - healed land is the second economy (P2.4-R). **Rung 1 is a *drip irrigator* that consumes nothing (P2.4-R3)**, fed by an incorporated Water Tank; the rungs are **multiblocks** built from a shared inert component vocabulary, and the framework is **shipped**. The Pump is teardown-only (out of a Washing Machine found in Bulky Waste), so rung 1 sits behind the teardown spine. **Saplings are never obtainable as items (P2.4-R2, shipped)** - the tree planter is the only source of trees, so a tree farm is a running machine rather than hand-replanting. |
