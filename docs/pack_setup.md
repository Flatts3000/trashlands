...
**Default Options is wired up as of 2026-09-08 (#65).** The files were generated by running the game
through devbridge and issuing `/defaultoptions saveAll`, then trimmed - never authored from outside,
which is how you ship a `defaultoptions` folder that silently does nothing. `pack/options.txt` is
deleted and `pack/config/defaultoptions/options.txt` replaces it; see the resource pack section
below for why it is two lines rather than the 127 the mod wrote.
...
**That line moved out of `pack/options.txt` and into `pack/config/defaultoptions/options.txt` on
2026-09-08 (#65), and `pack/options.txt` is gone.** Default Options now supplies it. The two files
would have fought otherwise: a shipped `options.txt` exists before the mod runs, so Default Options
would never have fired and would have stayed inert content.

**The shipped defaults file is two lines on purpose.** Running the mod's `saveAll` writes a 127-line
`options.txt` plus a 10 KB `keybindings.txt` capturing **every** client setting from whatever machine
ran it - fov, gamma, gui scale, render distance, volumes. Shipping that would impose one dev session's
preferences on every player. Tested instead: with only the two resource-pack lines present, the mod
applies those and leaves everything else to vanilla (verified - `fov`, `gamma`, `guiScale` and
`renderDistance` all came out at vanilla values). `keybindings.txt` is not shipped at all, because the
pack adds no keybinds and the file only restates vanilla.

**One thing this does not fix, despite what #65 assumed.** Defaults apply when `options.txt` is
*absent*, so a player who later disables the resource pack still keeps it disabled and the quest
book's Discord icon still breaks for them with no error. That trap is inherent to first-run defaults
and Default Options does not escape it; what changed is that the pack now has one mechanism instead
of two, and stopped dictating settings it never meant to.
