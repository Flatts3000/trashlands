...
| **FTB Quests** | The design's named quest engine, and the vehicle the narrative rides on. **Welcome** (10), **Salvage** (16), **Groundwork** (19) and **The Depths** (21), 66 quests, from the first Block of Garbage to a piece of coal. 63 of them shipped in v0.10.0; Welcome grew to 10 on 2026-09-08 to cover the household sprawl. The four shipped at 61 in v0.9.0; the Ancient Sculk pair (#53) took The Depths to 21 in v0.10.0. |
...
| Mod | Why it is in |
|---|---|
| **Default Options** | Ships client defaults as a mod instead of as a raw `options.txt` override. It handles options, keybinds, **resource pack selection** and server list separately, which is the exact problem this pack has. **Wired up 2026-09-08 (#65)**; `pack/options.txt` is gone and `pack/config/defaultoptions-common.toml` replaces it, using the journal-gated resource-pack handler rather than the install-time-only options copy. Pinned `side = "client"`: its entrypoint is `@Mod("defaultoptions", dist = [Dist.CLIENT])`, so on `side = "both"` it would ship into every server pack and never load. |
...
**Default Options is wired up as of 2026-09-08 (#65).** The config was generated by running the game
through devbridge and issuing `/defaultoptions saveAll` - never authored from outside, which is how
you ship a `defaultoptions` folder that silently does nothing. `pack/options.txt` is deleted and
`pack/config/defaultoptions-common.toml` replaces it. See the resource pack section below for why
that file and not the `config/defaultoptions/options.txt` the same command also wrote.
...
### The pack resource pack, and how it gets switched on
...
**A resource pack in `resourcepacks/` is not enabled by default**, so the pack forces it on:

```
resourcePacks:["vanilla","file/trashlands"]
```

**`pack/options.txt` is gone as of 2026-09-08 (#65). `pack/config/defaultoptions-common.toml`
replaces it**, with one field that matters:

```toml
defaultResourcePacks = ["vanilla", "file/trashlands"]
```

**Default Options has two separate mechanisms and only one of them is worth shipping.** The first
draft of this change picked the wrong one, so the difference is recorded:

- **The `OPTIONS` handler** copies `config/defaultoptions/options.txt` over a missing `options.txt`.
  Its gate is `!options.txt.exists()`, so it is **install-time only** - exactly the trap the raw
  override had. Running `saveAll` to produce that file also writes 127 lines capturing every client
  setting from whatever machine ran it (fov, gamma, gui scale, volumes) plus a 10 KB
  `keybindings.txt`, which would impose one dev session's preferences on every player. **Not shipped.**
- **The `resource-packs` handler** reads `defaultResourcePacks` from the Balm config above. Its gate
  is `hasDefaults(ctx) && !DefaultOptionsJournal.load(ctx).contains("resource-packs")` - the
  **journal**, `defaultoptions.journal.json`, not the presence of `options.txt`. **This is what the
  pack ships.**

**That gate is the actual improvement over the old override**, and it was verified rather than
argued. Two launches:

| Test | Setup | Result |
|---|---|---|
| A | no `options.txt`, no journal, only the shipped toml | `resourcePacks:["vanilla","file/trashlands","mod_resources"]` |
| B | existing `options.txt` with the pack **switched off**, journal cleared | pack re-enabled, `file/trashlands` in the resource reload |

B is the case the CurseForge override could never reach: a player already holding an `options.txt`.

**What it still does not do.** Once applied, the journal records `resource-packs` and the handler
never fires again, so a player who turns the pack off *after* that stays off. This applies the default
once per journal entry; it does not enforce it. `mod_resources` in the result is NeoForge's own pack
and is expected.
