...
6. **The components are inert, and this is load-bearing.** The Solar Panel does not detect light, emit redstone or generate power (**P3.5**: no RF before the Nether). The Pump does not move fluid, expose kinetics, or require Create (**P2.3**: "Recompile converts, Create moves"), and the Water Tank inside a spreader holds nothing. Their *names* invite exactly the opposite, so it is stated rather than assumed. The dripping spigots are a client-side particle on the formed machine, not the pump gaining mechanics.
