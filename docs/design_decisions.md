...
**As of 2026-07-24:** Rung 1 (the Grass Spreader) **built and shipped** - see **P2.4-R3** (a *drip irrigator* fed by an incorporated Water Tank; consumes nothing; machine-only; the rungs are **multiblocks** built from a shared inert component vocabulary; the Pump was teardown-only, out of a Washing Machine found in Bulky Waste, so rung 1 sat behind the teardown spine. **Corrected 2026-09-04:** the Pump is `blueprint_crafting` as well as a find, and as of Recompile 0.18.0 the crafted one costs Rubber Scrap, whose only source is a tire dump. Rung 1 now sits behind the blueprint and the rubber; the found Pump still exists and still teaches nothing. See `progression_gates.md`. Items 1, 5, 7 revised and item 8 **closed** in the build - the spreader and collector are deliberately siblings, not an ordered chain). P2.4 item 5 revised - see **P2.4-R** (healing pays out via the returning overworld; machine-per-rung ladder; global index rejected). P2.4 item 2's sapling clause superseded - see **P2.4-R2** (saplings are never obtainable; the tree planter is the only source of trees, **shipped**). P1.7 healed-land immunity revised - see **P1.7-R** (the junkyard fights back: healed *surface* is contested at the frontier, the rung ladder is the defence, mound retirement stays permanent; **shipped** as Recompile Phase 2.10 ahead of Phase 5). P0 fully locked (P0.1-P0.5). P1 fully locked (P1.1-P1.11; food/foraging tier P1.9 and the sorting-tarp mechanic revision both added in a later 2026-07-14 session; water P1.10 and Bulky Waste P1.11 added 2026-07-15, the latter superseding P1.1's appliance row; building-blocks tier P1.12 added and shipped 2026-07-16). **P2 fully locked (P2.1-P2.8);** the **Scrap Bin** (P2.9, storage) locked-and-shipped 2026-07-24; the **Scrap Network** (P2.10, an adjacency-connected local-logistics cluster of the scrap blocks - reversed 2026-07-24 from an earlier fixed-multiblock design) locked-and-built 2026-07-24. **The knowledge system (P1.4) is under review** - see the note at the head of P1.4; its enforcement model does not survive contact with modded autocrafting, and its scope is being questioned. Dimensions, the knowledge system, mound regrowth, the material economy ([`material_economy.md`](material_economy.md)), and the narrative layer ([`the_twist.md`](the_twist.md), spoilers) are locked. **Mod lineup: corrected 2026-09-04.** This line used to read "Create (belts/logistics), Mekanism (chemical/radiation endgame)" and was the most misleading sentence in the repo, because it is the current-state marker a reader trusts first. **Neither mod exists on MC 26.1.2** - no NeoForge build past 1.21.1 - so neither is in the pack and neither can be. The shipped lineup is 65 mods (`pack_setup.md`); the roles those two were holding are filled by **AE2 and Ender IO** for logistics and **Powah** for energy, with no chemical or radiation tier at all. Every doc that still argues from a Create or Mekanism premise is wrong on the evidence even where its conclusion survives. (Productive Frogs crossover dropped 2026-07-14.)
...
sentence of the paragraph below).** The **Trommel** sorts garbage at exactly the tarp's rolls per
block; both read one function (`SortableBlock.sortRolls`), so the two cannot drift apart. The reward
for building it is that it **runs unattended**, not that it yields more.

**The machine changed, the decision did not (2026-08-16, #187/#188).** Automated sorting was the
Separator's second mode for four releases and is now the Trommel's only job. A shear shredder destroys
distinctions and sorting requires one, so the machine that sorts is the one that makes a size cut. The
rate decision above is untouched - both machines always read the same function, and the Trommel still
does.
...
demolition yard, steel and power, so deleting it would leave bare hands as the only sorting from
...
yard's own piles sort into scrap and that scrap separates into gems, which closes the demolition-yard
loop - across **two** machines since #187, not inside one. That it took one machine was the argument
for giving the Separator a second verb, and it is the argument that lost.
...
0a. **Water holds the ground.** Wet farmland is spared; dry farmland is taken. An irrigated plot defends itself and an abandoned one dries out and goes back to the dump - so the P1.10 water economy becomes a reclamation *defence*, not only an input. This is the one place encroachment displaces player investment, and the severity is bounded: a crop on unwatered farmland comes off with the soil but is **dropped, not destroyed**, so you lose the plot and the growth but keep the seed. Gated behind "you let it dry out". Dirt paths stay out entirely.
0. **The whole dirt family is takeable, with two exceptions.** Not just grass - dirt, podzol, mud and moss all revert, so a half-finished conversion earns no free pass. **Mycelium is exempt**: it is the dump-mushroom substrate, and letting the junkyard eat it would quietly erode the only renewable food in the world (P1.9). Coarse dirt is exempt as the end state. Everything is driven off block tags built from *other* tags, so a mod adding dirt variants is covered without a mod release.
1. **Contested frontier only.** Coarse earth takes back soil that **borders unhealed ground**. Interior grass is safe until the front reaches it. Erosion therefore marches inward ring by ring - a visible advancing edge, which reads as a fight, where grass rotting at random would read as decay.
...
**CLOSED 2026-08-19 by Recompile 0.12.0 (pack v0.8.0).** This paragraph recorded a cost: the world preset's Nether entry was still stock vanilla, so the dimension supplied gold ore, quartz, glowstone, ancient debris, blaze rods and piglin bartering directly, routing around designed found sources in [`material_economy.md`](material_economy.md) - gold especially, which is gated behind E-Scrap to Circuit Powder to a Cupola blast. The themed build was named as the thing that would close it, and it has. The compacted depths generate no vanilla nether blocks at all; quartz, glowstone, nether wart, blaze powder and magma cream now come out of waste your machines process, and netherrack, basalt, blackstone and both nyliums are crafted from shards sorted out of slag rubble. Fortresses and bastions still generate, so blaze rods and bartering are untouched by design.
...
**Amendment (shipped 2026-07-16):** point 5 was overridden by the owner - the Rain Collector **ships standalone**, not gated on a consumer. Its job is producing **water bottles** (glass bottle in -> water bottle out) plus bucket/pipe transfer; washing salvage (point 6) stays the intended first *processing* consumer but is decoupled from shipping the block. It was built as a **real fluid tank** (26.1's new transfer `ResourceHandler` API, water-only), so it already plugs into the pump end of the ladder - and it is a 1x2x1 structure (solid base holding the tank + a draped tarp catching the rain). **The container is a found item:** empty glass bottles drop from household sorting at half the tin-can weight, so you rinse found bottles into water at the collector. **The reason given here was wrong and is corrected (2026-08-08, Recompile #161):** this said you cannot craft one because vanilla's recipe needs `minecraft:glass` and the world has only `cullet_glass`. The world has both - stone shards craft `minecraft:stone`, which crafts a vanilla furnace, which smelts the sand Reinforced Concrete drops. The bottle is uncraftable now because its recipe is explicitly disabled under P2.11, not because the material is missing. Scarcity is not enforcement; see P2.11 item 7. Open thread carried forward: water bottles have no vanilla consumer yet in this pack (brewing/thirst gated), so a downstream for water (washing, concrete-mixing, etc.) still needs picking.

**Amendment (2026-08-12, owner ruling): water is findable after all, once, and on purpose.** Point 1's founding fact - *this world contains no water at all* - now has a single deliberate exception. The **Dead Fridge** teardown rolls a secondary drop of snow or **`minecraft:ice`**, and ice is a water source: broken without silk touch on solid ground it leaves a water source block, and two sources are infinite water. So there is a route to water that needs no Rain Collector and no bucket.

This was raised as a defect and overruled with the consequence stated: *"I know it creates a water source. Its fine gameplay."* Recording it because P1.10's whole argument rests on the absence, and a future reader who finds ice in a teardown would otherwise correctly conclude it was a mistake and remove it.

**What survives is scale, and that is the line.** Water off a fridge costs a Bulky Waste find, a prybar and a 1-in-4 draw - it is a bounded reward at the end of a real chain. An *ambient* water source is still forbidden, which is why `leachate_is_not_water` is untouched: leachate answering yes to being water would turn every pool on the map into a tap. A bounded route is a reward; an ambient one ends the economy.
...
5. **RETIRED 2026-08-01 (owner).** ~~The invariant: nothing enters the found economy without a teardown exit. Finds in, materials out.~~ It had already stopped being true - the collectibles are found and displayed with no teardown exit, and the mod ships three teardown recipes in total. A find may exist to be *displayed* rather than processed (recovered paintings, recompile #99, are the case that forced the call). The reasoning below is kept because the *reason* still holds: a find that is neither useful nor wanted is clutter. What changed is that being worth looking at now counts.

   Original text: If that holds, quantity stops mattering: 1,000 mattresses is 4,000 string, 1,000 doors is 2,000 scrap. A pile is not clutter, it is **unprocessed inventory** - which is what a scrapyard is. This retires two systems designed earlier the same day: knowledge-aware "curriculum" loot (which needed the player in the loot context, and an answer to "what does a machine know?"), and blueprints-as-flood-control (they may still exist, but they are no longer load-bearing).
...
> **Items 1, 3 and 5 are superseded (2026-07-23).** Item 5: healing now pays out - see **P2.4-R**. Items 1 and 3 *for rung 1*: it consumes nothing and is machine-only - see **P2.4-R3**. Item 2's sapling clause: see **P2.4-R2**. Item 4 stands.

1. ~~**Healing is a recipe, not a right-click:** compost (from organic muck) + clean water + seed. Muck becomes the healing currency; leachate purification (clean water is made, not found) becomes load-bearing.~~ **SUPERSEDED for rung 1 (2026-07-23) by P2.4-R3** - the Grass Spreader consumes nothing; it carries an incorporated rain collector. Muck loses the healing-currency job; rungs 2-4 are not yet specced.
...
3. **Scale ladder:** ~~manual per-block first (the first grass patch is a monument), then~~ the tier-4/5 irrigator/soil-spreader (radius conversion ~~consuming compost + clean water~~ - how mound footprints retire at scale), region-scale healing reserved for the final chapter. **The manual-first clause is SUPERSEDED 2026-07-23 by P2.4-R3** - rung 1 is machine-only and the monument beat moves to *building* the machine. "Irrigator" turned out to be the right instinct - more literally than expected: it is a *drip* irrigator.
...
## P2.4-R3 - Rung 1 is a drip irrigator, and the rung ladder is multiblocks (locked 2026-07-23)

**Revises P2.4 items 1 and 3, and P2.4-R item 3, for rung 1.** Design work across the 2026-07-23 build sessions; recorded here because it had been living only in the mod repo's specs.

1. **Rung 1 is the Grass Spreader, and it is a *drip irrigator*.** Not a hopper that spreads soil - a tower ringed by four copper spigots that patiently drip **water** onto the surrounding ground, turning dead earth to grass within a radius. Drip rather than spray was settled during the build: a machine whose whole job is patient, deliberate reclamation should not read as a lawn sprinkler. The water comes from a **Water Tank built into its own structure**.
2. **It consumes nothing.** This supersedes "healing is a recipe, not a right-click: compost + clean water + seed" (P2.4 item 1) and "each consuming compost + clean water (+ power at tier)" (P2.4-R item 3), *for rung 1*. Three reasons it holds:
   - **The cost moved into the build.** The machine contains its own water supply, so the fiction explains the mechanic rather than hand-waving past it. One steep build, not a drip.
   - **P1.7-R already supplies the ongoing pressure.** When healed land was permanent, input cost was the only thing keeping healing honest; now the junkyard takes ground back, so the cost is *spatial* - a spreader must out-pace erosion, and permanence still needs trees.
   - **The declared inputs do not exist.** There is no compost item, and compost sits behind the P1.4 knowledge system, which is under review and may never land. A consuming rung 1 could not ship at all.
   
   **What this costs, honestly:** `organic_muck` loses the "healing currency" job P2.4 item 1 gave it, and leachate purification loses a headline consumer. (P1.10 item 6 had already ruled healing a *later* water consumer, not the first, so less is lost than it looks.)
3. **Rung 1 is machine-only**, superseding P2.4 item 3's "manual per-block first - the first grass patch is a monument." The monument beat moves to *building the machine*.
4. **The rung machines are multiblocks:** a core you place plus components stacked on it, formed in place, master/dummy - Immersive Engineering's pattern, trimmed. **Framework SHIPPED 2026-07-23** with the Rain Collector as its first consumer.
5. **Machines are assembled from a shared component vocabulary** - Machine Frame, Water Tank, Pump, Copper Pipe, Solar Panel - plus **one bespoke core per machine**. This is what keeps the art cost tractable: the vocabulary is paid for once, then each machine costs a core plus its formed appearance.
6. **The components are inert, and this is load-bearing.** The Solar Panel does not detect light, emit redstone or generate power (**P3.5**: no RF before the Nether). The Pump does not move fluid, expose kinetics, or require Create (**P2.3**: "Recompile converts, Create moves").

   **REVERSED for the Water Tank on 2026-08-18 (owner), and for that block only** (Recompile #229). A tank now holds water, takes a bucket and exposes a fluid capability. The reason is that it is the one component named for a **capacity** rather than an action: a pump that does not pump reads as a part, a tank that does not hold reads as broken - and two playtesters reported it as broken within ninety minutes of each other, one of them in the same sentence as a real bug, which is the cost that decided it. **The Grass Spreader still consumes nothing** (same ruling, explicitly), so a formed spreader may contain water it has no use for. A container is not a consumer. Their *names* invite exactly the opposite, so it is stated rather than assumed. The dripping spigots are a client-side particle on the formed machine, not the pump gaining mechanics.
7. **The Pump is teardown-only** - torn out of a **Washing Machine** found in Bulky Waste, never crafted - so **rung 1 sits behind the teardown spine and a find.** You salvage a pump before you can water anything. **Superseded 2026-09-04:** the Pump shipped as `blueprint_crafting` too, so "never crafted" was never true in the build, and Recompile 0.18.0 put **Rubber Scrap** in the recipe - a material with one source, the tire dumps. The find still exists and is still how most players meet it; it is no longer the only route.

   **Scoped reversal 2026-08-08 (owner ruling, Recompile #160): "never crafted" no longer holds; everything it was for does.** The Pump gains a **blueprint** route - tearing down a Washing Machine now also grants Idea Fragments toward a Pump blueprint, four of them, craftable only at the Scrap Crafting Table while the sheet is held or filed. Fragments come from nowhere but teardown, so **rung 1 still sits behind the teardown spine and a find**; what changes is that it takes four washing machines instead of one lucky one. That is a harder gate than the original, not a softer one - the wording was reversed to protect the clause after it.

   **The Pump is now the only blueprint that is also salvage**, and that is deliberate: a Clean Mattress and a Hydroponics Bay exist nowhere but the bench, a Pump exists in both places. Worth knowing because Recompile's `a_blueprint_result_has_no_other_route` sweep reads crafting recipes only and is blind to teardown, so it passes for the Pump without covering it; `ComponentBlueprintTests` pins the pairing instead.

   **The Motor and the Bulb do NOT get this yet, and the issue's premise that they would was wrong.** The pattern hangs a `teaches` entry on the teardown that yields the component - and neither the Motor nor the Bulb comes from a teardown at all. They drop from sorting (`mechanical_pulls` and `household_pulls`), where there is nothing to hang it on. Owner ruling the same day: give each a found object that tears down into it (a broken fan, a light fixture), which is new content and is filed separately.

   **Revised 2026-07-23 (in build).** This originally said *Motor*, out of a generic `broken_appliance`. Both were wrong, and the second one violated a locked decision: **P1.11 item 1 dropped the appliance precisely because it was "a vague abstraction sitting between the player and a specific thing"** - and this reintroduced it under the same name. A washing machine is the concrete object P1.11 asked for, it visibly pumps water out so the teardown needs no explanation, and it is the example this doc's own teardown-as-function note (P1.4) already reaches for. Not split into dishwasher/refrigerator: three appliances only pay off when they tear down into *different* things, and the trigger is a second machine needing a second part.
8. ~~**Your first machine becomes part of your second.** The spreader incorporates a *literal* Rain Collector, so the water thread is ordered: no collector, no spreader.~~ **SUPERSEDED 2026-07-23, and the machines are deliberately siblings. CLOSED 2026-07-24.**

   Two things broke it. A machine may not take another machine's core as a component (nesting cores makes the inner one try to assemble itself into cells the outer one has claimed), so the cell became an inert **Water Tank**. Then the recipe direction was corrected - a Rain Collector *contains* a tank, so building a tank out of a collector had the dependency backwards - making the tank the primitive and the collector `tank + copper pipe`.

   Two changes retired the ordering. A machine may not take another machine's core as a component (nesting cores makes the inner one assemble itself into the outer one's cells), so the cell became an inert **Water Tank**. And the tank is the shared primitive - a Rain Collector is `tank + copper pipe`, so building a tank *out of* a collector had the dependency backwards. The Water Tank is therefore craftable from raw materials, and **a Grass Spreader does not require a Rain Collector anywhere in its chain**.

   **This is the settled shape, not a gap.** The two are parallel machines built from the same tank, not an ordered chain, and rung 1's gate is what it should be: the **Pump** (a Bulky Waste find, or the blueprint - see item 7's 2026-09-04 correction, which adds rubber from a tire dump to the crafted route) and the multiblock build, plus **copper** behind the Burn Barrel. "No collector, no spreader" is retired, not pending - a machine that shows you a tank the moment you can build one is a cleaner read than one that makes you build a whole collector first to disassemble it.
9. **No wood, anywhere.** This world has no trees, so nothing in a machine may read as lumber (the pallet under the collector's tote is compressed junk, not a wooden skid).

**Art strategy that came out of this and is worth keeping:** reuse vanilla models and textures wherever they fit, and derive new textures by **deterministic recolouring** rather than drawing. The Rain Collector's tarp funnel *is* the vanilla hopper (its funnel shape lives in `hopper_inside`, recoloured to tarp blue); the Solar Panel is a recoloured daylight detector. This directly attacks the art gate, which is the real constraint on the mod.
...
2. **REVERSED 2026-07-31: RF no longer starts here.** The energy tier moves forward, ahead of the Nether, arriving with the hydroponics tier as a burner generator plus a working Solar Panel. Recompile itself only **speaks** Forge Energy - `Capabilities.Energy.BLOCK` is a NeoForge standard, not a Mekanism feature, so a machine can consume FE with zero mod dependencies and any energy mod interoperates for free. **When the player gets power therefore becomes a pack decision** (which energy mod Trashlands ships, and when) rather than an engine one, which is far cheaper to change than this locked entry was.
   - **What this costs: the Nether loses its stated reason to exist.** "First RF power originates here" was the draw. Osmium, the netherite-analog from the compacted depths, and nether-tier salvage remain, but whether those carry a whole dimension on their own is now **open and needs answering before P3.5 is built.**
   - **The inert-components rule narrows but survives.** The Solar Panel stops being a prop and becomes a real generator. The **Pump stays inert** - that is P2.3 ("Recompile converts, Create moves"), a different decision, untouched by this.
   - Superseded text, kept for the record: *"The reason to go: first RF power and osmium originate here... before the Nether, no RF (fuel/manual only, consistent with 'no energy at tier 2'). The Nether unlocks RF generation."*
