...
## P2.4-R3 - Rung 1 is a sprinkler, and the rung ladder is multiblocks (locked 2026-07-23)
...
1. **Rung 1 is the Grass Spreader, and it is a *sprinkler*.** Not a hopper that spreads soil - a tower that constantly throws **water** over the surrounding ground, turning dead earth to grass within a radius. The water comes from a **Rain Collector built into its own structure**.
...
5. **Machines are assembled from a shared component vocabulary** - Machine Frame, Motor, Solar Panel - plus **one bespoke core per machine**. This is what keeps the art cost tractable: the vocabulary is paid for once, then each machine costs a core plus its formed appearance.
6. **The components are inert, and this is load-bearing.** The Solar Panel does not detect light, emit redstone or generate power (**P3.5**: no RF before the Nether). The Motor does not rotate anything, expose kinetics, or require Create (**P2.3**: "Recompile converts, Create moves"). Their *names* invite exactly the opposite, so it is stated rather than assumed. A spinning sprinkler head is a client-side visual on the formed machine, not the motor gaining mechanics.
7. **The Motor is teardown-only** - torn out of a `broken_appliance` found in Bulky Waste, never crafted - so **rung 1 sits behind the teardown spine and a find.** You salvage a motor before you can water anything. This also restores the appliance P1.11 dropped when Bulky Waste replaced it, as a *find item* this time.
8. **Your first machine becomes part of your second.** The spreader incorporates a *literal* Rain Collector, so the water thread is ordered: no collector, no spreader.
