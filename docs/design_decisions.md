...
8. ~~**Your first machine becomes part of your second.** The spreader incorporates a *literal* Rain Collector, so the water thread is ordered: no collector, no spreader.~~ **SUPERSEDED 2026-07-23 (in build), and the ordering is currently LOST.**
   ...
   The consequence, stated plainly: **the Water Tank is craftable from raw materials, so a Grass Spreader no longer requires a Rain Collector anywhere in its chain.** The collector is now a sibling, not an ancestor. If the water thread should still be ordered, that has to be put back deliberately - the obvious lever is the spreader core's recipe. **Open.**
