...
PROSE_OUT = "docs/quest_prose.md"    # --prose-only default; gitignored
...
def main() -> int:
    ...
    ap.add_argument("--out", default=DEFAULT_OUT,
                    help="output path, or - for stdout (default: " + DEFAULT_OUT
                         + ", or " + PROSE_OUT + " with --prose-only)")
    args = ap.parse_args()

    if args.split:
        return write_split(args.split)

    # --prose-only must not default to DEFAULT_OUT. quest_book.md is tracked
    # and says "Do not edit this file" in its own first line; running
    # --prose-only without --out silently replaced it with the stripped prose
    # document, which then read as a legitimate regeneration in the diff.
    out = args.out
    if args.prose_only and out == DEFAULT_OUT:
        out = PROSE_OUT
    ...
    with open(out, "w", encoding="utf-8", newline="\n") as fh:
        fh.write(text)
    print("wrote {} ({} lines)".format(out, len(text.splitlines())))
    return 0
