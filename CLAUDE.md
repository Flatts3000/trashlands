...
**Status: alpha, shipping (v0.4.0, 2026-08-05).** The pack is **assembled and pinned** - 47 mods, MC 26.1.2 / NeoForge 26.1.2.94 - and the tag-driven release pipeline is in (`.github/workflows/release.yml` -> GitHub release + CurseForge upload). CurseForge project **`1636627`**, slug `trashlands`; `CF_PROJECT_ID` and `CF_API_TOKEN` are set on the repo. Recompile is at v0.7.0 and released on CurseForge (project `1625740`).
...
## Next actions
1. **Chapter three** - the Scrap Network and storage (issue #11). The teaching problem is that the network is invisible from the items: nothing tells a player that placing blocks touching each other makes them one system.
