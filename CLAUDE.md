...
## Next actions
1. **Chapter three** - the Scrap Network and storage (issue #11). **Re-read the issue before starting it.** The adjacency rule is taught inside Salvage as of v0.5.0, so what was left is Scrap Bins bound to one material, the Filing Cabinet, and crafting straight out of the cluster - but the 2026-08-20 storage additions mean that chapter would now teach the weakest of four storage systems. A drawer bound to one item is a Scrap Bin. What this chapter is *for* is an open question, not a writing task.
