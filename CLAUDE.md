...
## Next actions
1. **Playtest v0.9.0.** Nothing in it has been launched by a person. The pack proves it loads, boots
   and generates the right world, and the book validates, but fifteen mods, a new chapter and two
   Recompile majors have never been played. Two specific things need eyes: that the Simple Magnets
   recipes show Magnet Scrap in JEI rather than lapis (the override wins on a load-order line and
   fails silently), and that a sewer crate actually yields the AE2 presses.
