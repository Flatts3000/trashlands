...
Crash Utilities, Better Compatibility Checker), and the auto-pulled libraries. Full table and the considered-but-cut list are in `docs/pack_setup.md`.
...
## Next actions
1. **Playtest v0.12.0.** Nothing in the last three releases has been launched by a person. The pack
   proves it loads, boots, generates the right world and validates its book, which is not the same
   thing. The backlog of unplayed content is now seventeen mods, two chapters' worth of quests and
   six Recompile majors (0.13.0 through 0.18.0). Specific things that need eyes: that the Simple Magnets recipes show
